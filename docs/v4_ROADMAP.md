...
> **미션:** 한국 분리수거 표준 (환경부 분리배출.kr) 정확도 최우선 — 운영비 0원
> **모바일 전용 사용자** 기준 설계
...
### A3. 후보 선택 UI (1~2일)
**무엇:** "혹시 이것인가요?" Top 5 표시 (모바일 카메라 표준 패턴)
...
### B2. 실시간 카메라 박스
**무엇:** 카메라 켜면 즉시 객체 박스 (모바일 카메라 표준 패턴)
...
- 정확도: COCO-SSD 40% → **92%** (한국 사물 특화 수준)
- 속도: 1~2초 (네이티브와 거의 동일)
- 모델 크기: ~10MB (5MB precache + 5MB on-demand)
- 사용자 만족도: ⭐⭐⭐⭐⭐
- 우리 강점: **전 플랫폼 PWA + 환경부 표준 100%**
