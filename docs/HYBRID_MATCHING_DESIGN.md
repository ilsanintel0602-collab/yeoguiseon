...
**목표**: 일상 폐기물 85% 케이스 ms 단위 응답 + Gemini 비용 -85% (RecycleAI 94.2% pass@1 모범)
...
- RecycleAI 94.2% 같은 정량 평가
- 효과: 85% 케이스를 Gemini 없이 처리 — RecycleAI 동등 정확도·속도

---

## RecycleAI 차이점

| 영역 | RecycleAI | 여기선 v6.0 (목표) |
|---|---|---|
...
## 사용자 영역 지정 강화 (RecycleAI 모범)

RecycleAI: 녹색 박스(AI 자동) + 주황 박스(사용자 드래그)

여기선 현재: "영역을 손가락으로 표시" 안내 + 영역 캡처

개선 방향:
- 사용자 드래그 박스 = 주황색 표시 (RecycleAI 차별화 UX)
